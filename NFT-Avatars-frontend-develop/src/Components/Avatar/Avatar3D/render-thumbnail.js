    ...
    attributeNames.map((name) => {
      ...
      return [
        ...
        new THREE.BufferAttribute(
          new Float32Array((verts.count / 2) * itemSize),
          itemSize
        ),
      ];
    })
...
function shouldBisect(categoryName, partValue) {
  const part = assets[categoryName].parts.find(
    (part) => part.value === partValue
  );
...
export function renderThumbnail(
  ...
  camera.position.copy(
    new THREE.Vector3().copy(center).add(unitZ.multiplyScalar(size.z))
  );
