...
import constants from "../Avatar3D/constants";
import { GLTFCubicSplineInterpolant } from "./gltf-cubic-spline-interpolant";

function mergeMorphTargetInfluences({
  meshes,
  ...
  Object.entries(destMorphTargetDictionary).map(([morphName, destIndex]) => {
    ...
    destMorphTargetInfluences[destIndex] =
      mesh.morphTargetInfluences[sourceIndex];
...
function mergeSourceAttributes({ sourceAttributes }) {
  ...
  Array.from(propertyNames.keys()).map((name) => {
    destAttributes[name] = BufferGeometryUtils.mergeBufferAttributes(
      allSourceAttributes
        .map((sourceAttributes) => sourceAttributes[name])
        .flat()
    );
  });
...
function mergeSourceMorphTargetDictionaries({ sourceMorphTargetDictionaries }) {
  ...
  const allSourceDictionaries = Array.from(
    sourceMorphTargetDictionaries.values()
  );
...
function mergeSourceMorphAttributes({
  ...
  allSourceMorphAttributes.forEach((sourceMorphAttributes) => {
    Object.keys(sourceMorphAttributes).forEach((name) =>
  ...
  propertyNames.forEach((propName) => {
    unmerged[propName] = [];
    Object.entries(destMorphTargetDictionary).forEach(
      ([morphName, destMorphIndex]) => {
        ...
        meshes.forEach((mesh) => {
          ...
          if (morphTargetDictionary.hasOwnProperty(morphName)) {
            ...
            bufferAttribute =
              mesh.geometry.morphAttributes[propName][sourceMorphIndex];
          } else {
            const attribute = mesh.geometry.attributes[propName];
            const array = new attribute.array.constructor(
              new Array(attribute.array.length).fill(0)
            );
  ...
  propertyNames.forEach((propName) => {
    ...
    Object.entries(destMorphTargetDictionary).forEach(
      ([morphName, destMorphIndex]) => {
        merged[propName][destMorphIndex] =
...
function remapMorphTrack({
  track,
  sourceMorphTargetDictionary,
  destMorphTargetDictionary,
}) {
  const sourceOffsets = CubicSplineFrameOffsets({
    numMorphs: Object.keys(sourceMorphTargetDictionary).length,
  });
  const destOffsets = CubicSplineFrameOffsets({
    numMorphs: Object.keys(destMorphTargetDictionary).length,
  });
  ...
  for (let frameIndex = 0; frameIndex < numFrames; frameIndex++) {
    ...
    destMorphNames.forEach((morphName) => {
      ...
      const isMorphInSourceTrack =
        sourceMorphTargetDictionary.hasOwnProperty(morphName);
      if (isMorphInSourceTrack) {
        const sourceMorphIndex = sourceMorphTargetDictionary[morphName];
        destFrame[destOffsets.tanIn + destMorphIndex] =
          sourceFrame[sourceOffsets.tanIn + sourceMorphIndex];
        destFrame[destOffsets.value + destMorphIndex] =
          sourceFrame[sourceOffsets.value + sourceMorphIndex];
        destFrame[destOffsets.tanOut + destMorphIndex] =
          sourceFrame[sourceOffsets.tanOut + sourceMorphIndex];
      } else {
  ...
  const destTrack = new THREE.NumberKeyframeTrack(
    destTrackName,
    track.times,
    destKeyframes.flat()
  );
...
export function mergeGeometry({ meshes }) {
  const source = {
    ...
    morphAttributes: new Map(
      meshes.map((m) => [m, m.geometry.morphAttributes])
    ),
    morphTargetDictionaries: new Map(
      meshes.map((m) => [m, m.morphTargetDictionary || {}])
    ),
    morphTargetInfluences: new Map(
      meshes.map((m) => [m, m.morphTargetInfluences || []])
    ),
    animationClips: new Map(
      meshes.map((m) => [m, findSceneGroup(m).animations])
    ),
  };

  const dest = {};
  dest.attributes = mergeSourceAttributes({
  ...
  dest.animations = remapAnimationClips({
    meshes,
    animationClips: dedupBy(
      Array.from(source.animationClips.values()).flat(),
      "name"
    ),
    sourceMorphTargetDictionaries: source.morphTargetDictionaries,
    destMorphTargetDictionary,
