function chooseWeightedRandom(parts) {
  ...
  return partsWithWeightInfo.parts.find(
    (info) => randomNum >= info.minValue && randomNum < info.maxValue
  );
}
...
function maybeReplaceWithMatchingCategory(assets, newConfig, categoryName) {
  ...
  const partToMatch = assets[categoryNameToMatch].parts.find(
    (part) => part.value === newConfig[categoryNameToMatch]
  );
  ...
  const currentPart = category.parts.find(
    (part) => part.value === newConfig[categoryName]
  );
  ...
  const matchingAsset = category.parts.find((part) => {
    return (
      ...
      part.description[primaryOption] ===
        partToMatch.description[primaryOption] &&
      part.description[secondaryOption] ===
        currentPart.description[secondaryOption]
    );
  });
...
export function generateRandomConfig(assets) {
  ...
  for (const categoryName of categoryNames) {
    const categoryAssets = assets[categoryName].parts.filter(
      (part) => !part.excludeFromRandomize
    );
