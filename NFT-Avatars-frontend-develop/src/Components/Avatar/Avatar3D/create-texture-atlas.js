  ...
  return async function createTextureAtlas({ meshes }) {
    const contexts = Object.fromEntries(
      IMAGE_NAMES.map((name) => [
        name,
        createContext({ width: ATLAS_SIZE_PX, height: ATLAS_SIZE_PX }),
      ])
    );
    ...
    const uvs = new Map(
      meshes.map((mesh, i) => {
        const min = new THREE.Vector2(
          i % numTiles,
          Math.floor(i / numTiles)
        ).multiplyScalar(1 / numTiles);
      ...
      await Promise.all(
        IMAGE_NAMES.map(async (name) => {
          const texture = new THREE.Texture(
            await canvasToImage(contexts[name].canvas)
          );
