...
function saveInitialMorphTargetInfluences(node) {
  node.traverse((obj) => {
    if (
      ...
      !obj.userData.initialMorphTargetInfluences
    ) {
      obj.userData.initialMorphTargetInfluences =
        obj.morphTargetInfluences.slice(0);
    }
  });
}
  ...
  for (const mesh of findChildrenByType(node, "SkinnedMesh")) {
    ...
    for (let i = 0; i < mesh.morphTargetInfluences.length; i++) {
      mesh.morphTargetInfluences[i] =
        mesh.userData.initialMorphTargetInfluences[i];
    }
  }
}
...
function applyMorphRelationships(part) {
  ...
  for (const {
    ...
    targetMorphName,
    targetMorphValue,
  } of part.morphRelationships) {
    const mesh = state.avatarNodes[targetCategoryName].getObjectByProperty(
      "type",
      "SkinnedMesh"
    );
...
function tick(time) {
...

  {
    if (state.shouldApplyNewAvatarConfig) {
      ...
      const categories = new Set(
        Object.keys(state.newAvatarConfig).concat(
          Object.keys(state.avatarConfig)
        )
      );
      ...
      for (const category of categories) {
        ...
        if (state.newAvatarConfig[category] !== state.avatarConfig[category]) {
          ...
          if (state.newAvatarConfig[category] !== null) {
            loadIntoGroup({
              category,
              part: state.newAvatarConfig[category],
              group: state.avatarNodes[category],
            }).then(() => {
...

  {
    if (state.shouldApplyMorphRelationships) {
      ...
      for (const categoryName of Object.keys(state.avatarConfig)) {
        if (!state.avatarConfig[categoryName] || !assets[categoryName])
          continue;

        resetMorphTargetInfluences(state.avatarNodes[categoryName]);

        const currentPart = assets[categoryName].parts.find(
          (part) => part.value === state.avatarConfig[categoryName]
        );
...

  {
    ...
    const didCameraMove = !prevCameraPosition.equals(
