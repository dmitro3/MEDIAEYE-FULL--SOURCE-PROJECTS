...
import { findChildrenByType, findChildByName } from "./utils";
...
export function cloneSkeleton(skinnedMesh) {
  ...
  return new THREE.Skeleton(
    skinnedMesh.skeleton.bones.map((b) => boneClones.get(b))
  );
}
...
export const exportGLTF = (function () {
  ...
  return function exportGLTF(object3D, { binary, animations }) {
    return new Promise((resolve) => {
      exporter.parse(object3D, (gltf) => resolve({ gltf }), {
        binary,
        animations,
      });
    });
...
function cloneIntoAvatar(avatarGroup) {
  ...
  for (const avatarRoot of avatarRoots) {
    clonedAvatarRoot.userData = combineHubsComponents(
      clonedAvatarRoot.userData,
      avatarRoot.userData
    );
  }

  // Clone skinned meshes, bind them to a new skeleton
  const clonedSkinnedMeshes = findChildrenByType(
    avatarGroup,
    "SkinnedMesh"
  ).map((o) => {
...
export async function exportAvatar(avatarGroup) {
  ...
  if (debugConfig.debugGLTF) {
    // const { gltf } = await exportGLTF(avatar, {
    //   binary: false,
    //   animations: avatar.animations,
    // });
  }

  const { gltf: glb } = await exportGLTF(avatar, {
    binary: true,
    animations: avatar.animations,
  });
  return { glb };
