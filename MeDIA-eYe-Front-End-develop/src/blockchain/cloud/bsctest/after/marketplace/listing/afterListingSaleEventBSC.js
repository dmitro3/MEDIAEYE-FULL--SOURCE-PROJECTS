Moralis.Cloud.afterSave('EventBSCListingSaleTest', async (request) => {
  ...
  logger.info(JSON.stringify(listingNFTs));
  ...
  for (let i = 0; i < listingNFTs.length; i++) {
    ...
    newSale.set('chain', 'BSC');
    newSale.set('currency', 'BNB');
    ...
    userSpendingQuery.equalTo('chain', '0x61');
    ...
    if (userSpending !== undefined) {
      logger.info('user spending ' + JSON.stringify(userSpending));
      ...
      newUserSpending.set('chain', '0x61');
