Moralis.Cloud.afterSave('EventFTMFeaturePaid', async (request) => {
  ...
  logger.info(
    '--------------------------------After Featuring Collection FTM--------------------------------'
  );
  ...
  const setCommon = (featuredObject) => {
    featureObject.set('chainId', '0xfa');
  ...
  switch (DEFINITIONS[featureType]['name']) {
    case 'NFT':
      const FeaturedNFTs = Moralis.Object.extend('Featured');
    ...
    case 'collection':
      const FeaturedCollections = Moralis.Object.extend('Featured');
