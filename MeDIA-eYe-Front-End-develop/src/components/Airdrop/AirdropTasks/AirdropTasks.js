...
import { toggleSocialPopup } from '../../../store/app/appSlice';
...
import { queryAirdropTask } from '../../../blockchain/functions/Airdrops';
...
const AirdropTasks = (props) => {
  ...
  const { completedTasks, setCompletedTasks } = useContext(AirdropContext);
      ...
      for (let task in airdrop?.attributes?.tasks[media]) {
        renderList.push(
          <AirdropTaskAction
            ...
            switchPopup={switchSocialPopup}
            switchCommentPopup={switchSocialCommentPopup}
            completed={
              completedTasks?.[`${media}`]?.[`${task}`]
                ? completedTasks[media][task].attributes?.status
                : false
            }
            refreshTasks={checkCompletedTasks}
          />
        );
      }
    }
  ...
  useEffect(() => {
    if (airdrop?.attributes?.tasks && Moralis) {
      checkCompletedTasks();
    }
  }, [airdrop, Moralis]);
  const checkCompletedTasks = async () => {
    ...
    let params = {
      airdropObjectId: airdrop?.id,
      user: user?.attributes?.ethAddress
    };
    const res = await queryAirdropTask(Moralis, params);
    // construct dict mapping for completed tasks by user

    let completedTasks = {};
    res?.map((completedTask) => {
      const media = completedTask?.attributes?.media;
      if (!(typeof completedTasks[media] === 'object')) {
        completedTasks[media] = {};
      }
      completedTasks[media][completedTask?.attributes?.task] = completedTask;
    });
  ...
  const [followclubhouse, setfollowclubhouse] = useState('rohit.sharma3');
  const [joinclubhousechannel, setjoinclubhousechannel] = useState(
    'https://www.clubhouse.com/room/PAdpWOQ6?utm_medium=ch_room_tr&utm_campaign=Z842XcaboqnXFXFu-jOKBQ-290738'
  );
  const [joinclubhouseclub, setjoinclubhouseclub] = useState(
    'https://www.clubhouse.com/club/testingclub-123?utm_medium=ch_club&utm_campaign=Z842XcaboqnXFXFu-jOKBQ-290741'
  );
  ...
  const switchSocialPopup = async (currTask, currMedia) => {
    // set current task on popup switch
    if (currTask) setCurrTask(currTask);
    if (currMedia) setCurrMedia(currMedia);
  ...
  const switchSocialCommentPopup = async (currTask, currMedia, comment) => {
    ...
    if (showCommentPopup === true) {
      ...
      } else if (title === 'Flickr Post Comment') {
        try {
          let apidata = await getFlickrAPiUrl();
          if (apidata && apidata.key === 1) {
            localStorage.setItem('flickrsecretkey', apidata.requestSecret);
    ...
    try {
      ...
      if (apidata.key === 1) {
        dispatch(
          toggleGeneralPopup({
            status: 'success',
            message: apidata.display_name.message,
            textButton: 'OK',
            size: 'sm'
          })
        );
  ...
  const follow_clubhouse = () => {
    var url = `https://www.clubhouse.com/@${followclubhouse}`;
    window.open(url);
  };
  const join_clubhouse_channel = () => {
    window.open(joinclubhousechannel);
  };
  const join_clubhouse_club = () => {
    window.open(joinclubhouseclub);
  };
  ...
  const follow_soundcloud = async () => {
    var url = `https://soundcloud.com/${followsoundcloud}`;
    window.open(url);
  };
    ...
    <div className="">
      ...
      <section className="mediaeye-layout-section">
        <div className="mediaeye-task">
          <div className="mediaeye-task-header">
            <label>Tasks</label>
            <label className="mediaeye-task-header-right">
              13 Tasks <span>to complete</span>
            </label>
          </div>

          {
            ...
            airdrop?.attributes?.tasks ? renderTasks() : null
          }
          {/* <div>
            {' '}
          ...
          <div className="mediaeye-task-row">
            ...
            <div className="mediaeye-task-row-inner">
              <h6>Retweet last Twitter Pinned post</h6>
              <button
                type="button"
                className="btn btn-info"
                onClick={() => {
          ...
          <div className="mediaeye-task-row">
            ...
            <div className="mediaeye-task-row-inner">
              ...
              <button
                ...
                onClick={() => {
                  setTxt('Submit');
                  setTitle('Join Clubhouse Channel');
                  switchSocialPopup();
                  join_clubhouse_channel();
                }}
              >
                Join
              </button>
