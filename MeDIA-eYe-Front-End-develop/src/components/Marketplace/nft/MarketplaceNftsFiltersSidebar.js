...
const MarketplaceNftsFiltersSidebar = (props) => {
    ...
    useEffect(() => {
        ...
        if (activeNetwork === 'BSC') {
            list = [{ name: 'WBNB', value: 'WBNB' }, { name: 'BUSD', value: 'BUSD' }];
        } else if (activeNetwork === 'ETH') {
            list = [{ name: 'WETH', value: 'WETH' }, { name: 'USDT', value: 'USDT' }];
        } else if (activeNetwork === 'FTM') {
            list = [{ name: 'WFTM', value: 'WFTM' }, { name: 'USDC', value: 'USDC' }];
        }
    ...
    useEffect(() => {
        ...
        if (selectedFilters?.status && allFilters?.status) {
            for (let i = 0; i < allFilters?.status?.length; i++) {
                if (selectedFilters.status.indexOf(filterList?.status[i].value) !== -1) {
        ...
        if (selectedFilters?.category && allFilters?.category) {
            for (let i = 0; i < allFilters?.category?.length; i++) {
                if (selectedFilters.category.indexOf(allFilters?.category[i].value) !== -1) {
        ...
        if (selectedFilters?.collections && allFilters?.collections) {
            for (let i = 0; i < allFilters?.collections?.length; i++) {
                if (selectedFilters.collections.indexOf(allFilters?.collections[i].value) !== -1) {
    ...
    useEffect(() => {
        if (removeItem) {
            let allFilters = { ...filterList };
            if (removeItem?.reset || removeItem?.all) {
                for (let property in allFilters) {
                    for (let i = 0; i < allFilters[property].length; i++) {
                        if (property === 'quantity' && allFilters[property][i].value === 'All') {
    ...
    return (
        <div className="marketplace-page-sidebar mediaeyefancyScroll" ref={sidebarRef} style={{ maxWidth: sidebarWidth ? sidebarWidth + 'px' : '' }}>
            <div className="marketplace-page-sidebar-inner">
                <div className="marketplace-page-sidebar-col">
                    <div className={`marketplace-page-sidebar-col-heading ${statusToggle ? 'active' : ''}`} onClick={() => setStatusToggle(!statusToggle)}>Status <Angle side="small_down" /></div>
                    {
                        statusToggle ? (
                            filterList?.status.map((list, i) => (
                                <div key={i} className={`marketplace-page-sidebar-col-row ${list?.selected ? 'active' : ''}`} onClick={(e) => { updateFilterValue('status', list.value, true) }}>
                ...
                <div className="marketplace-page-sidebar-col">
                    <div className={`marketplace-page-sidebar-col-heading ${priceToggle ? 'active' : ''}`} onClick={() => setPriceToggle(!priceToggle)}>Price <Angle side="small_down" /></div>
                    {
                        priceToggle ? (
                            <>
                                <div className='marketplace-page-sidebar-col-price'>
                                    <div className='marketplace-page-sidebar-col-price-col'><input className='mediaeyeform-input marketplace-page-sidebar-col-price-input' value={filters?.price.min} placeholder='Min' onChange={(e) => { applyPrice('min', e.target.value) }} /></div>
                                    <div className='marketplace-page-sidebar-col-price-col'>to</div>
                                    <div className='marketplace-page-sidebar-col-price-col'><input className='mediaeyeform-input  marketplace-page-sidebar-col-price-input' value={filters?.price.max} placeholder='Max' onChange={(e) => { applyPrice('max', e.target.value) }} /></div>
                                    <div className='marketplace-page-sidebar-col-price-col'><SelectSearch className="mediaeye-selectsearch mediaeye-selectsearch-white-arrow dark-style" value={filters?.price.network} options={priceTokenList} onChange={(opt) => { applyPrice('network', opt) }} /></div>
                                </div>
                                <div className='marketplace-page-sidebar-col-row'>
                                    <button type="button" className='btn btn-info btn-square btn-block m-t-5'>Apply</button>
                                </div>
                            </>
                ...
                <div className="marketplace-page-sidebar-col">
                    <div className={`marketplace-page-sidebar-col-heading ${quantityToggle ? 'active' : ''}`} onClick={() => setQuantityToggle(!quantityToggle)}>Quantity <Angle side="small_down" /></div>
                    {quantityToggle ? (
                        filterList?.quantity.map((list, i) => (
                            <div key={i} className={`marketplace-page-sidebar-col-row ${list?.selected ? 'active' : ''}`} onClick={(e) => { updateFilterValue('quantity', list.value) }}>
                                <div className='marketplace-page-sidebar-col-row-icon'><Radiobox type={list?.selected} /></div>
                                <div className='marketplace-page-sidebar-col-row-name'>{list.name}</div>
                            </div>
                        ))
                    ) : null
                ...
                <div className="marketplace-page-sidebar-col">
                    <div className={`marketplace-page-sidebar-col-heading ${collectionsToggle ? 'active' : ''}`} onClick={() => setCollectionsToggle(!collectionsToggle)}>Collections <Angle side="small_down" /></div>
                    {
                        collectionsToggle ? (
                            <>
                                ...
                                <div className="marketplace-page-sidebar-col-headingsub">Top 100</div>
                                {filterList?.collections.map((list, i) => (
                                    <div key={i} className={`marketplace-page-sidebar-col-row ${list?.selected ? 'active' : ''}`} onClick={(e) => { updateFilterValue('collections', list.value, true) }}>
                                        <div className='marketplace-page-sidebar-col-row-icon'><SelectMark type={list?.selected} /></div>
                                        <div className='marketplace-page-sidebar-col-row-name'>{list.name}</div>
                                    </div>
                                ))
                                }
                ...
                <div className="marketplace-page-sidebar-col">
                    <div className={`marketplace-page-sidebar-col-heading ${categoryToggle ? 'active' : ''}`} onClick={() => setCategoryToggle(!categoryToggle)}>Category <Angle side="small_down" /></div>
                    {categoryToggle ? (
                        filterList?.category.map((list, i) => (
                            <div key={i} className={`marketplace-page-sidebar-col-row ${list?.selected ? 'active' : ''}`} onClick={(e) => { updateFilterValue('category', list.value, true) }}>
                                <div className='marketplace-page-sidebar-col-row-icon'><Checkbox type={list?.selected} /></div>
                                <div className='marketplace-page-sidebar-col-row-name'>{list.name}</div>
                            </div>
                        ))
                ...
                <div className="marketplace-page-sidebar-col">
                    <div className={`marketplace-page-sidebar-col-heading ${titleToggle ? 'active' : ''}`} onClick={() => setTitleToggle(!titleToggle)}>Title <Angle side="small_down" /></div>
                    {titleToggle ? (filterList?.title.map((list, i) => (
                        <div key={i} className={`marketplace-page-sidebar-col-row ${list?.selected ? 'active' : ''}`} onClick={(e) => { updateFilterValue('title', list.value, true) }}>
                            <div className='marketplace-page-sidebar-col-row-icon'><Checkbox type={list?.selected} /></div>
                            <div className='marketplace-page-sidebar-col-row-name'>{list.name}</div>
                        </div>
                    ))) : null
...
export default MarketplaceNftsFiltersSidebar;
